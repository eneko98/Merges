#!/bin/bash
#SBATCH --job-name=eval_linear_regular
...
#SBATCH --array=0-1%4
#SBATCH --output=/scratch/evalero/projects/merge_llm/evaluation/logs/out/merges/qwen/linear/14B/linear_regular_%A_%a.out
#SBATCH --error=/scratch/evalero/projects/merge_llm/evaluation/logs/err/merges/qwen/linear/14B/linear_regular_%A_%a.err
...
echo "Running REGULAR tasks for LINEAR merge: ${model_name}"

############ OUTPUT DIRECTORIES ############
base_dir="/scratch/evalero/projects/merge_llm/evaluation"
results_dir="${base_dir}/results/merges/qwen/linear/${model_name}"
mkdir -p "$results_dir"
