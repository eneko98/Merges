#!/bin/bash
#SBATCH --job-name=eval_ties_flores
...
#SBATCH --array=0-0%4
#SBATCH --output=/scratch/evalero/projects/merge_llm/evaluation/logs/out/merges/llama/ties/ties_flores_%A_%a.out
#SBATCH --error=/scratch/evalero/projects/merge_llm/evaluation/logs/err/merges/llama/ties/ties_flores_%A_%a.err
...
############ MODELS ############
MODELS=(
  "ties_EU100_INS100|/data/evalero/projects/merge_llm/merges/models/ties/llama/ties_EU100_INS100"
)
...
echo "Running FLORES tasks for TIES merge: ${model_name}"

############ OUTPUT DIRS ############
base_dir="/scratch/evalero/projects/merge_llm/evaluation"
results_dir="${base_dir}/results/merges/llama/ties/${model_name}/flores"
mkdir -p "$results_dir"
