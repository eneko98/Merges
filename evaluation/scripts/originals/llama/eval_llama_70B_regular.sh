#!/bin/bash
#SBATCH --job-name=eval_llama_70B_regular
...
#SBATCH --array=0-1%1
#SBATCH --output=/scratch/evalero/projects/merge_llm/evaluation/logs/out/originals/llama/llama-3.1-70B-Instruct_regular_%A_%a.out
#SBATCH --error=/scratch/evalero/projects/merge_llm/evaluation/logs/err/originals/llama/llama-3.1-70B-Instruct_regular_%A_%a.err
...
for task in "${tasks_selected[@]}"; do
  ...
  if [[ "$task" == "eus_reading" ]]; then
    model_args="pretrained=${model_path},dtype=bfloat16,tensor_parallel_size=4,max_model_len=18192,gpu_memory_utilization=0.5,max_num_seqs=1"
  else
    model_args="pretrained=${model_path},dtype=bfloat16,tensor_parallel_size=4,max_model_len=18192,gpu_memory_utilization=0.9,max_num_seqs=1"
  fi
