#!/bin/bash
#SBATCH --job-name=eval_llama_8B_regular
...
#SBATCH --array=0-6%4
#SBATCH --output=/scratch/evalero/projects/merge_llm/evaluation/logs/out/originals/llama/eval_llama_8B_regular_%A_%a.out
#SBATCH --error=/scratch/evalero/projects/merge_llm/evaluation/logs/err/originals/llama/eval_llama_8B_regular_%A_%a.err
...
LM_HARNESS_VENV="/scratch/evalero/envs/vllm017"
...
############ MODELS TO EVALUATE ############
# Format: "model_name|model_path|kind"
# kind = base  -> no chat template flags
# kind = instr -> use --apply_chat_template --fewshot_as_multiturn

MODELS=(
  # === BASE FINE-TUNES (local) ===
  "cat_Llama-3.1-8B|/data/evalero/models_original/cat_Llama-3.1-8B|base"
  "es_Llama-3.1-8B|/data/evalero/models_original/es_Llama-3.1-8B|base"
  "gl_Llama-3.1-8B|/data/evalero/models_original/gl_Llama-3.1-8B|base"
  "eu_Llama-3.1-8B|/data/evalero/models_original/Latxa3.1_8b_lr1e-5|base"

  # === ORIGINAL BASE MODEL (HF) ===
  "Llama-3.1-8B|meta-llama/Llama-3.1-8B|base"

  # === INSTRUCT MODELS ===
  "Llama-3.1-8B-Instruct|meta-llama/Llama-3.1-8B-Instruct|instr"
  "Latxa-Llama-3.1-8B-Instruct|HiTZ/Latxa-Llama-3.1-8B-Instruct|instr"
)
...
results_dir="${base_dir}/results/originals/llama/${model_name}"
mkdir -p "$results_dir"
