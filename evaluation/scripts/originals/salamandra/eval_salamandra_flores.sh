#!/bin/bash
#SBATCH --job-name=eval_salamandra_flores
...
#SBATCH --array=0-1%4
#SBATCH --output=/scratch/evalero/projects/merge_llm/evaluation/logs/out/originals/salamandra/eval_salamandra_flores_%A_%a.out
#SBATCH --error=/scratch/evalero/projects/merge_llm/evaluation/logs/err/originals/salamandra/eval_salamandra_flores_%A_%a.err
...
results_dir="${base_dir}/results/originals/salamandra/${model_name}/flores"
mkdir -p "$results_dir"
...
for task in "${tasks_selected[@]}"; do
  ...
  model_args="pretrained=${model_path},dtype=bfloat16,tensor_parallel_size=1,gpu_memory_utilization=0.8,max_num_seqs=1"
